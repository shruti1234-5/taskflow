...
export default function EmployeeSidebar({ setSidebarWidth, isMobileOpen, setIsMobileOpen }) {
 ...
 const router = useRouter();
  ...
  // Toggle sidebar and update layout margin
  const toggleSidebar = () => {
    if (isMobile) {
  ...
  const menuItems = [
    { name: "Dashboard", href: "/employee/dashboard", icon: "mdi:view-dashboard" },
    { name: "Assign Task", href: "/employee/assign-tasks", icon: "mdi:clipboard-text" },
    { name: "My Tasks", href: "/employee/my-tasks", icon: "mdi:format-list-bulleted" },
  ];
  ...
  return (
    <>
      ...
      {isMobile && isMobileOpen && (
        <div
          className="fixed inset-0 z-40 md:hidden bg-white/10 dark:bg-black/20"
      ...
      <aside
        className={`
          ...
          dark:bg-gradient-to-b dark:from-gray-900 dark:to-[#0f0f11] dark:text-gray-100
        `}
      >
      {/* Logo + Toggle */}
      <div>
        <div className="flex items-center justify-between mb-6">
          ...
          >
            <Icon icon={isMobile ? (isMobileOpen ? "mdi:close" : "mdi:menu") : (isOpen ? "mdi:chevron-left" : "mdi:chevron-right")} width="22" className="text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200 transition-colors" />
          </button>
        </div>
        ...
        <nav className="flex flex-col gap-2">
          {menuItems.map((item) => (
            <Link
              ...
              className={`
                flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 transform
                ${
                  pathname === item.href
